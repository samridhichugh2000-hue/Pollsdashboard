...
export default function ApprovePage({
  ...
  const [mode, setMode] = useState<'view' | 'edit' | 'reject' | 'feedback'>(initialMode)
  const [doneAction, setDoneAction] = useState<DoneAction>(null)
  ...
  const submit = async (action: 'approve' | 'save_and_approve' | 'reject' | 'feedback') => {
  ...
  if (status === 'done') {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4" style={{ background: gradient }}>
        <div className="w-full max-w-md rounded-3xl bg-white px-8 py-10 shadow-2xl text-center">
          <div className={`mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full ${cfg.bg}`}>{cfg.icon}</div>
          <h2 className="text-2xl font-bold text-gray-900">{cfg.title}</h2>
          <p className="mt-3 text-sm text-gray-500 leading-relaxed">{cfg.message}</p>
          <p className="mt-8 text-xs text-gray-300">Koenig Solutions HR · Poll Management System</p>
        </div>
      </div>
    )
      ...
      <div className="mx-auto w-full max-w-2xl">
        ...
        <div className="rounded-3xl bg-white shadow-2xl px-7 py-7 space-y-6">
          ...
          {mode === 'view' && (
            <>
              <div>
                ...
                <p className="rounded-lg bg-gray-50 px-4 py-2.5 text-sm font-medium text-gray-800">{poll.subject ?? `Poll: ${poll.topic}`}</p>
              </div>
              {poll.draft_email_body && (
                <div>
                  <p className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-gray-400">Draft Email Body</p>
                  <div className="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{poll.draft_email_body}</div>
                </div>
              )}
              {questions.length > 0 && (
                <div>
              ...
              {poll.ms_form_link && (
                <div>
                  ...
                  <a href={poll.ms_form_link} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline break-all">{poll.ms_form_link}</a>
                </div>
              )}
            </>
          )}
          ...
          {mode === 'edit' && (
            <>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-400">Subject Line</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-400">Email Body</label>
                <textarea value={editEmailBody} onChange={(e) => setEditEmailBody(e.target.value)} rows={8}
          ...
          {mode === 'view' && (
            <div className="space-y-3">
              <div className="flex flex-col gap-3 sm:flex-row">
                <button onClick={() => submit('approve')} disabled={status === 'submitting'}
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <button onClick={() => setMode('feedback')}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl border-2 border-amber-500 py-3 text-sm font-semibold text-amber-700 hover:bg-amber-50 transition-colors">
                  <MessageSquare className="h-4 w-4" /> Feedback
                </button>
                <button onClick={() => setMode('reject')}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl border-2 border-red-500 py-3 text-sm font-semibold text-red-600 hover:bg-red-50 transition-colors">
          ...
          {mode === 'edit' && (
            <div className="flex flex-col gap-3 sm:flex-row">
              <button onClick={() => submit('save_and_approve')} disabled={status === 'submitting'}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3 text-sm font-semibold text-white shadow-lg hover:bg-emerald-700 disabled:opacity-60 transition-colors">
                {status === 'submitting' ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
          ...
          {mode === 'feedback' && (
            <div className="space-y-3">
              <div>
                ...
                <textarea value={feedbackRemarks} onChange={(e) => setFeedbackRemarks(e.target.value)} rows={4}
                  placeholder="Describe the changes needed before this poll can be approved..."
                  className="w-full rounded-xl border border-amber-200 px-3.5 py-2.5 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition resize-none" />
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <button onClick={() => submit('feedback')} disabled={status === 'submitting' || !feedbackRemarks.trim()}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-amber-500 py-3 text-sm font-semibold text-white shadow-lg hover:bg-amber-600 disabled:opacity-60 transition-colors">
                  {status === 'submitting' ? <Loader2 className="h-4 w-4 animate-spin" /> : <MessageSquare className="h-4 w-4" />}
                  Send Feedback
                </button>
                <button onClick={() => setMode('view')}
                  className="flex-1 rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4}
                  placeholder="Please explain why this poll request is being rejected..."
                  className="w-full rounded-xl border border-red-200 px-3.5 py-2.5 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition resize-none" />
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <button onClick={() => submit('reject')} disabled={status === 'submitting' || !rejectReason.trim()}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-red-600 py-3 text-sm font-semibold text-white shadow-lg hover:bg-red-700 disabled:opacity-60 transition-colors">
                  {status === 'submitting' ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4" />}
                  Confirm Rejection
                </button>
                <button onClick={() => setMode('view')}
                  className="flex-1 rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
          ...
          <p className="text-center text-xs text-gray-300">Koenig Solutions HR · Poll Management System</p>
        </div>
      </div>
    </div>
  )
