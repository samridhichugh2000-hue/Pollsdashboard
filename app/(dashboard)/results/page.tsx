                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-1.5 mt-1.5 flex-wrap">
                          {entry.actionable === true && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">Actionable</span>
                          )}
                          {entry.actionable === false && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">Not Actionable</span>
                          )}
                          {cls === 'rms' && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">RMS</span>
                          )}
                          {cls === 'non_rms' && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-purple-100 text-purple-700">Non-RMS</span>
                          )}
                          {cls === 'partial' && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">Partial</span>
                          )}
                          {currentStatus(i) === 'wip' && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">WIP</span>
                          )}
                          {currentStatus(i) === 'completed' && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 text-green-700">Completed</span>
                          )}
                          {entry.reply_sent_at && (
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">Replied</span>
                          )}
                        </div>
                      </div>
                      ...
                      {isSaving ? (
                        ...
                        <div className="flex flex-wrap gap-1.5 flex-shrink-0 justify-end">
                          <button
                            onClick={() => void handleActionable(i, entry.actionable === true ? null : true)}
                          ...
                          <button
                            onClick={() => void handleActionable(i, entry.actionable === false ? null : false)}
                            disabled={savingIndex !== null}
                            className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold border transition-colors ${
                              entry.actionable === false
                          ...
                          <button
                            onClick={() => void handleStatus(i, 'wip')}
                            disabled={savingIndex !== null}
                            className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold border transition-colors ${
                              currentStatus(i) === 'wip'
                          ...
                          <button
                            onClick={() => void handleStatus(i, 'completed')}
                            disabled={savingIndex !== null}
                            className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold border transition-colors ${
                              currentStatus(i) === 'completed'
                    ...
                    {isExpanded && (
                      <div className="mt-4 ml-10 space-y-4">
                              ...
                              return (
                                <button
                                  ...
                                  className={`px-4 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
                                    isActive ? CLS_CONFIG[key].active : CLS_CONFIG[key].inactive
                                  }`}
                                >
                        ...
                        <div className="space-y-1.5">
                          ...
                          />
                          <button
                            disabled={savingRemarksIndex === i}
                            onClick={() => void handleSaveRemarks(i)}
                        ...
                        <div className="space-y-1.5 pt-1 border-t border-gray-100">
                          ...
                          />
                          <button
                            disabled={sendingReplyIndex === i || !entryReplies[i]?.trim()}
                            onClick={() => void handleSendReply(i)}
...
export default function ResultsPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Results & Follow-up</h2>
            ...
            {polls.map(poll => (
              <div key={poll.id} className="hover:bg-gray-50 transition-colors">
                ...
                <div className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-4 px-5 py-3">
                  ...
                  <button
                    onClick={() => handleUploadClick(poll)}
                    disabled={uploadingId === poll.id}
                ...
                <div className="px-5 pb-3">
                  <button
                    onClick={() => void handlePushToRms(poll)}
                    disabled={pushingRmsId === poll.id}
