...
export async function GET(req: Request) {
  ...
  for (const poll of reminderSentPolls) {
    ...
    // deadline that lands on a weekend meant this cron's own weekend-skip
    // above silently and permanently lost the alert, since it never got a
    // later day where the dates matched exactly. This still only fires once
    // per poll (claimPollColumn below), it just isn't tied to one exact day.
