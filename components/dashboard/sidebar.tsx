...
export function Sidebar() {
  ...
  return (
    <aside
      ...
      className="flex h-full flex-col border-r border-slate-100 bg-white dark:border-slate-800 dark:bg-[#161b27] py-5 flex-shrink-0 overflow-hidden"
    >
      {/* Logo + collapse toggle */}
      <div className={cn('mb-6 flex items-center px-3', collapsed ? 'justify-center flex-col gap-2' : 'gap-3 px-4')}>
        {!collapsed && (
        ...
        <button
          ...
          className={cn(
            'flex-shrink-0 rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-200 transition-colors',
            collapsed && 'mt-1'
          )}
        >
      ...
      {/* Main nav */}
      <nav className={cn('flex flex-1 flex-col gap-0.5', collapsed ? 'px-1.5' : 'px-3')}>
      ...
      <div className={collapsed ? 'px-1.5' : 'px-3'}>
        ...
        {settingsNav.map(item => <NavItem key={item.href} {...item} collapsed={collapsed} />)}
      </div>
