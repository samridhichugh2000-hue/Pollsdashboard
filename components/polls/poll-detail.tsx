...
export function PollDetail({ poll: initialPoll, approvals, auditLogs, response: initialResponse }: PollDetailProps) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {['CLOSED', 'RESULTS_UPLOADED', 'RESULTS_SHARED'].includes(poll.status) && (
                <>
                  <Button
                    className="w-full"
                    size="sm"
      ...
      {['SENT', 'REMINDER_SENT', 'CLOSED', 'RESULTS_UPLOADED', 'RESULTS_SHARED', 'ARCHIVED', 'RMS_PUBLISHED'].includes(poll.status) && (() => {
        type EntryType = {
          email?: string
          respondent?: string
          submitted_at: string
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (confirm('Close pending responses? Anyone whose response was never marked actionable or not-actionable will get an automatic "no action taken" email, and those entries will be marked resolved. Do this only after you\'ve finished acting on individual responses.')) {
                          void runAction('CLOSE_PENDING_RESPONSES').then(() => void refreshResponse())
                        }
                      }}
                      disabled={!!loading}
                    >
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      { label: poll.request_type === 'KGT' ? 'Finalised' : 'Actionable', value: stats.actionable, cls: 'bg-emerald-100 text-emerald-700' },
                      { label: poll.request_type === 'KGT' ? 'Not Finalised' : 'Not Actionable', value: stats.notActionable, cls: 'bg-slate-100 text-slate-600' },
